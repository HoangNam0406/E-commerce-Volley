...
<body class="bg-gray-50 flex flex-col min-h-screen">
    ...
    $msg = getFlashMessage(); ?>
            ...
            <div class="bg-white rounded-lg shadow max-w-2xl p-6">
                <form action="<?php echo APP_URL; ?>/admin/category-edit/<?php echo $data['category']['id']; ?>" method="POST" class="space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Category Name *</label>
                        <input type="text" name="name" value="<?php echo htmlspecialchars($data['category']['name']); ?>" required class="w-full px-4 py-2 border rounded-lg focus:ring-red-500 focus:border-red-500">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                        <textarea name="description" rows="4" class="w-full px-4 py-2 border rounded-lg focus:ring-red-500 focus:border-red-500"><?php echo htmlspecialchars($data['category']['description'] ?? ''); ?></textarea>
                    </div>

                    <div class="pt-4 border-t flex gap-4">
                        <button type="submit" class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700">Update Category</button>
                        <a href="<?php echo APP_URL; ?>/admin/categories" class="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-200">Cancel</a>
                    </div>
                </form>
            </div>
        </main>
